...
class DatabasePipeline(GooglePlayAdvancedSearch.DBUtils.AppSaver):
	...
	def process_item(self, item, spider):
		self.insertOrUpdateApp(item, self._AppSaver__freshDays)
		return item

	def open_spider(self, spider):
		l = ','.join(['\'' + id + '\'' for id in spider.targetAppIds])
		freshAppIds = [r[0] for r in self._AppSaver__cursor.execute(f"select id from app where julianday('now')-julianday(updateDate)<{self._AppSaver__freshDays} and id in ({l})")]
